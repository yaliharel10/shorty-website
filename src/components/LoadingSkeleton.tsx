  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
      {Array.from({ length: 12 }).map((_, i) => (
        <div key={i} className="animate-pulse overflow-hidden rounded-xl bg-[#141414]">
          <div className="aspect-[2/3] bg-[#1a1a1a]" />
          <div className="space-y-2 p-3">
            <div className="h-3 rounded bg-[#222]" />
...
export function FilmRowSkeleton() {
  return (
    <div className="mb-8 px-4 md:px-8 lg:px-12">
      <div className="mb-4 h-6 w-40 animate-pulse rounded bg-[#1a1a1a]" />
      <div className="flex gap-4 overflow-hidden">
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="h-[240px] w-[160px] shrink-0 animate-pulse rounded-lg bg-[#1a1a1a] sm:w-[180px] md:w-[200px]"
          />
        ))}
      </div>
    </div>
...
export function HeroSkeleton() {
  return (
    <div className="relative h-[55vh] min-h-[400px] max-h-[600px] w-full animate-pulse bg-[#111]">
